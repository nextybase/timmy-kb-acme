...
| `finito` | STATE_LABEL | `_run_summary` completa `README/SUMMARY` | `onboarding_full` (Git push) / `pipeline.github_utils` | `onboarding_full` (preflight + push) | **out-of-scope: agency (Timmy)** | `pytest`, `pre-commit` fail → Skeptic Gate blocca; `pipeline.github_utils` lancia `phase_failed` |
...
## Gaps & TODO
- Il gap residuo riguarda la connessione esplicita tra QA Gate e stato `finito` e la policy di retry automatizzato; occorre documentarle ulteriormente nelle istruzioni/06-08.
