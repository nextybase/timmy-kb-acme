...
from pipeline.drive_utils import (
    ...
    list_drive_files,  # 📦 per cercare file già presenti
    delete_drive_file  # 📦 per eliminare file già presenti
)
from pipeline.exceptions import PipelineError, ConfigError
from pipeline.context import ClientContext
from pipeline.path_utils import is_safe_subpath, sanitize_filename, is_valid_slug

# ---------------------------
# Utility interna per input/parametri
# ---------------------------
def get_or_prompt(value, prompt, non_interactive=False):
    """Ritorna il valore se già fornito, altrimenti chiede input o solleva errore in non-interactive."""
    if value:
        return value
    if non_interactive:
        raise ConfigError(f"Parametro mancante: {prompt}")
    return input(prompt)

def validate_slug(slug):
    """Valida slug secondo la regex configurata, altrimenti solleva errore."""
...
def update_config_with_drive_ids(context: ClientContext, new_data: dict, logger) -> None:
    ...
    if not config_path.exists():
        raise ConfigError(f"Config cliente non trovata: {config_path}", slug=context.slug, file_path=config_path)

    if not is_safe_subpath(config_path, context.base_dir):
        raise ConfigError(f"Path config non sicuro: {config_path}", slug=context.slug, file_path=config_path)
...
def ensure_cartelle_raw_yaml_exists(yaml_path: Path, logger, slug=None):
    ...
    if template_path.exists():
        ...
        try:
            ...
            raise PipelineError(f"Errore creazione YAML struttura cartelle: {e}", slug=slug, file_path=yaml_path)
...
def pre_onboarding_main(slug: str, client_name: str = None, no_interactive: bool = False, dry_run: bool = False):
    ...
    # 🆕 Gestione interattiva/non-interattiva
    slug = validate_slug(get_or_prompt(slug, "Inserisci slug cliente: ", non_interactive=no_interactive))
    client_name = get_or_prompt(client_name, "Inserisci nome cliente: ", non_interactive=no_interactive)
    ...
    try:
        # Fix percorso YAML
        struttura_file = context.settings.get("local_structure_file", "cartelle_raw.yaml")
        if struttura_file.startswith("config/"):
            struttura_file = struttura_file.replace("config/", "")
            logger.warning(f"🛠 Corretto percorso local_structure_file: {struttura_file}")

        yaml_path = Path("config") / struttura_file
        ensure_cartelle_raw_yaml_exists(yaml_path, logger, slug=context.slug)

        # 📁 Creazione struttura locale semplice
        ...
        # 🗑 Pulizia eventuale config.yaml già presente su Drive
        ...
        for f in existing_files:
            ...
            logger.info(f"🗑 Config precedente rimosso da Drive: {f['id']}")
        ...
        logger.error(f"⚠️ Errore pre-onboarding: {e}")
        raise
    except Exception as e:
        logger.error(f"🔥 Errore imprevisto: {e}", exc_info=True)
        raise PipelineError(e, slug=slug)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pre-onboarding Timmy-KB (v1.0 stabile)")
    ...
    parser.add_argument("--non-interactive", action="store_true", help="Disabilita input manuali")
    ...
    try:
        pre_onboarding_main(
            ...
            no_interactive=args.non_interactive,
