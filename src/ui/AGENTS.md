...
# Rules (overrides)
- Ensure `normalize_raw` runs before Semantica so `normalized/` is the input surface for tagging and conversion.
...
- Gate the Semantica tab so it is enabled only when `normalized/` is present locally.
...
# Acceptance Criteria
- Never trigger Semantica actions if `normalized/` is empty or missing.
