...
def _semantic_path(slug: str, filename: str) -> Path:
    ...
    return cast(Path, ensure_within_and_resolve(semantic_dir, semantic_dir / filename))


def _read_yaml_text(slug: str, filename: str) -> str:
    path = _semantic_path(slug, filename)
    if not path.exists():
        raise ConfigError(f"File `{path}` non trovato. Genera gli artefatti e riprova.")
    layout = _require_layout(slug)
...
def edit_semantic_mapping(slug: str) -> None:
    def _body() -> None:
        ...
        st.caption(
            ...
            "Usa YAML valido. Il salvataggio e atomico con guardie di path-safety."
        )
