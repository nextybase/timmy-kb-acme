...
def load_tags_by_category(path: str = "config/timmy_tags.yaml") -> Dict[str, List[str]]:
    """
    Carica i tag semantici da YAML strutturato per categoria.
    """
    try:
...
def markdownize_pdf_text(raw_text: str, tags_by_cat: Optional[Dict[str, List[str]]] = None) -> str:
    """
    Trasforma testo PDF in Markdown arricchito di heading e tag semantici.
    """
...
def extract_file_to_markdown(
    ...
    """
    Wrapper per supportare futuri altri formati (.docx, .png, ecc.)
    """
...
def convert_pdfs_to_markdown(
    ...
    """
    Converte ricorsivamente tutti i PDF nella cartella pdf_root in file Markdown nella cartella md_output_path.
    """
