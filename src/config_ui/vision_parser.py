...
def _is_plausible_heading(line: str) -> bool:
    ...
    if _hr_re.match(s):
        return True
    low = s.lower()
    if low in {"executive summary", "valori", "pilastri strategici", "offerta",
               "mercati prioritari", "operating model", "roadmap ad alto livello",
               "metriche chiave", "rischi & mitigazioni", "appendice", "appendix",
    ...
    if not words or len(words) > 8:
        return False
    cap = sum(1 for w in words if w.isupper() or (w[0].isupper() and (len(w) == 1 or w[1:].islower())))
...
def _clean_section_body(text: str) -> str:
    ...
    for l in lines:
        if not l:
            cleaned.append("")
            continue
        if re.fullmatch(r"\d{1,3}", l):  # numeri pagina
            continue
        if re.fullmatch(r"[•·●◦◉]", l):  # bullet isolati
            continue
        if _hr_re.match(l):
            continue
        cleaned.append(l)
...
def extract_ovm_sections(text: str) -> Dict[str, str]:
    ...
    for i, (start_idx, key) in enumerate(ovm_positions):
        end_idx = len(lines)
