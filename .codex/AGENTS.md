...
- Path-safety: write only within `src/`, `tests/`, `docs/`, `.codex/`; **eccezione limitata**: `tools/dummy/**` e `tools/smoke/**` **solo** per header/docstring standardizzati di confinamento (documentazione-only, nessuna logica) e `tools/ai_checks/**` per diagnostica CLI (no runtime/UI import); never touch `config/**`, `.env*`, `output/**`; scrivo solo in italiano salvo eccezione control-mode OCP -> Codex in inglese.
