...
- Coerenza: una sola fonte della verità per path/I-O (SSoT) e per i tag (SQLite in `semantic/tags.db`).
- Sicurezza: nessuna scrittura fuori dal perimetro cliente; maschera segreti nei log.
- Portabilità: supporto Win/Linux; attenzione a encoding e path (POSIX vs Windows) nei file scambiati.
