...
# Arricchimento semantico (estrazione tag ÔåÆ stub + YAML)
...
if not slug:
    ...
    if not clients:
        ...
        st.html('<a href="/new?tab=new" target="_self">Ô×ò Crea nuovo cliente</a>')
    ...
    for client in clients:
        ...
        label = f"{name} ({slug_value}) ÔÇö {state}"
...
# Da qui in poi: slug presente ÔåÆ viste operative
...
# --- Genera README / Rileva PDF / Scarica da Drive ÔåÆ locale in 3 colonne ---
    ...
    if emit_fn is None:
        ...
        try:
            ...
            with status_guard(
                ...
                "Genero i README nelle sottocartelle di raw/ su DriveÔÇª",
...
# Colonna 2 ÔÇö Arricchimento semantico (estrazione tag)
...
# Colonna 3 ÔÇô Scarica da Drive ÔåÆ locale

if _column_button(c3, "Scarica PDF da Drive ÔåÆ locale", key="btn_drive_download", type="secondary", width="stretch"):
    ...
    def _modal() -> None:
        ...
        if conflicts:
            ...
            with st.expander(f"File gi├á presenti in locale ({len(conflicts)})", expanded=True):
        ...
        if cB.button("Procedi e scarica", key="dl_proceed", type="primary", width="stretch"):
            ...
            try:
                ...
                try:
                    ...
                    st.toast("Allineamento DriveÔåÆlocale completato.")
