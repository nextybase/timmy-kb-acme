...
def main(argv: list[str]) -> int:
    ...
    for f in files:
        ...
        if not any(str(f).startswith(p) for p in ("src/", "tests/", "tools/")):
            continue
        if not has_spdx_header(f):
            missing.append(f)
    if missing:
        sys.stderr.write("Missing SPDX header in:\n" + "\n".join(f"  - {m}" for m in missing) + "\n")
        sys.stderr.write(f'Add a line near the top, e.g.: "# {SPDX_TOKEN}: GPL-3.0-only"\n')
        return 1
    return 0
