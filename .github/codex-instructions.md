...
- Documentation must be updated alongside any behavior change; consult the relevant docs (Developer Guide, Coding Rules, docs/AGENTS_INDEX.md, etc.).
...
- The agent interaction follows the OCP⇄Codex turn-based model described in `docs/PromptChain_spec.md`: Onboarding → meta analysis → operational prompts → final QA → Prompt N+1 report.
...
- When in doubt, reference `docs/AGENTS_INDEX.md` and the local AGENTS documents to determine the active policy for the area being modified.
