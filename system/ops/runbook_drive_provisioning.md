...
3. **Fase C – Provisioning Drive + publish README (manuale)**  
   ...
   - Receive la SSoT da `semantic_mapping.yaml` e da `system/specs/workspace_layout.v1.yaml` per costruire la struttura `raw/<area.key>` su Drive prima di pubblicare i README.  
   - Drive è *capability esplicita* e non un prerequisito per Fase A/B; si attiva solo dopo il completamento locale delle fasi precedenti.
...
- L’intera sequenza è deterministica, non introduce scaffold temporanei o shim.

## Out of Scope

- Non crea workspace locale (Fase A già gestita altrove).  
- Non genera VisionStatement né semantic_mapping (Fase B).  
...
- **Comportamento**: il sistema fallisce fast, logga l’errore strutturato e non lascia warning silenti né soft-recovery.  
...
- Questo modello è vincolante per il core: nessun warning skip, nessun fallback “graceful” nel processo di publishing.
