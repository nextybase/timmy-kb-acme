...
@pytest.mark.usefixtures("_stable_env")
def test_preview_stub_absolute_log_dir_invalid_path_errors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    st_stub = install_streamlit_stub(monkeypatch)
    monkeypatch.setattr("ui.utils.stubs.get_streamlit", lambda: st_stub, raising=True)
    monkeypatch.setattr("ui.chrome.render_chrome_then_require", lambda **_k: DUMMY_SLUG, raising=True)
...
def test_preview_stub_missing_log_dir_env_errors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    st_stub = install_streamlit_stub(monkeypatch)
    monkeypatch.setattr("ui.utils.stubs.get_streamlit", lambda: st_stub, raising=True)
    monkeypatch.setattr("ui.chrome.render_chrome_then_require", lambda **_k: DUMMY_SLUG, raising=True)
...
def test_preview_stub_missing_log_dir_path_errors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    st_stub = install_streamlit_stub(monkeypatch)
    monkeypatch.setattr("ui.utils.stubs.get_streamlit", lambda: st_stub, raising=True)
    monkeypatch.setattr("ui.chrome.render_chrome_then_require", lambda **_k: DUMMY_SLUG, raising=True)
