...
def test_topk_k_zero(monkeypatch, tmp_path: Path):
    ...
    out = retr.search(
        QueryParams(tmp_path / "kb.sqlite", "p", "s", "q", k=0, candidate_limit=10),
        FakeEmb(),
    )
    assert out == []
...
def test_topk_small_k_with_ties_stable(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(retr, "fetch_candidates", _mk_stub_candidates(contents, scores))
    out = retr.search(
        QueryParams(tmp_path / "kb.sqlite", "p", "s", "q", k=2, candidate_limit=10),
        FakeEmb(),
    )
...
def test_topk_k_one(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(retr, "fetch_candidates", _mk_stub_candidates(contents, scores))
    out = retr.search(
        QueryParams(tmp_path / "kb.sqlite", "p", "s", "q", k=1, candidate_limit=10),
        FakeEmb(),
    )
...
def test_topk_k_ge_n_behaves_like_full_sort(monkeypatch, tmp_path: Path):
    ...
    scores = [0.8, 0.8, 0.9]
    monkeypatch.setattr(retr, "fetch_candidates", _mk_stub_candidates(contents, scores))
    params = QueryParams(tmp_path / "kb.sqlite", "p", "s", "q", k=10, candidate_limit=10)
