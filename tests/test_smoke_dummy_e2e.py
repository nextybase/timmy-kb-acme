...
def test_smoke_dummy_e2e(tmp_path: Path, monkeypatch):
    ...
    run([PY, "src/pre_onboarding.py", "--slug", slug, "--name", "Cliente Dummy", "--non-interactive", "--dry-run"])
    ...
    run([PY, "src/tag_onboarding.py", "--slug", slug, "--source", "local", "--non-interactive", "--proceed"])
