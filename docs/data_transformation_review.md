...
- **Conversione RAW → Markdown**: `semantic.convert_service.convert_markdown` converte i PDF in `.md` con frontmatter semplificato e ora inserisce un `excerpt` estratto dal PDF, mantenuto da cache per evitare riscritture inutili, oltre a lasciare il placeholder contestuale per la tracciabilità.
...
- È stata introdotta una proposta ER persistente (`semantic/layout_proposal.yaml`) che conserva la struttura suggerita da `layout_enricher`, pronta per essere utilizzata dalla UI o ad ulteriori arricchimenti.
...
- Collegare la proposta ER alla generazione di tag/entità (frontmatter) e automatizzare la pubblicazione su GitBook/Drive, mantenendo i controlli di path-safety e scritture atomiche.
