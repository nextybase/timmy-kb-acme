# Onboarding UI – Guida aggiornata (v1.9.1)
...
## 14) Tabella comparativa: CLI vs UI
| Aspetto         | CLI (Orchestratori)                                         | UI (Streamlit)                                |
|-----------------|--------------------------------------------------------------|-----------------------------------------------|
| Entry point     | `pre_onboarding.py`, `tag_onboarding.py`, `onboarding_full` | `onboarding_ui.py` con tre tab                |
| Destinatari     | DevOps / CI / power user                                    | Utenti operativi / facilitatori               |
| Config          | YAML + variabili ENV                                        | Editor interattivo mapping + Drive provisioning |
| Semantica       | Script invocano `semantic.api.*`                            | UI invoca `semantic.api.*`                    |
| Preview         | Script `adapters/preview` opzionale                         | Bottoni UI (start/stop container Docker)      |
| Requisiti extra | Token GitHub (push finale)                                  | Docker attivo (preview), credenziali Drive    |
| Retrocompat     | Mirroring mapping YAML in `pre_onboarding` (flag)           | YAML solo come input storico, deprecato       |

---

> **Versione**: 2025-09-17
> **Stato**: Allineata al codice corrente, chiarito ruolo YAML (retrocompat), aggiunta comparativa CLI vs UI.
