# Coding Rules – Timmy‑KB (docs/coding_rule.md)

> **Scopo**
> Queste regole assicurano codice coerente, manutenibile e sicuro per la pipeline Timmy‑KB. Valgono per tutto il repository, con enfasi su orchestratori (`src/pre_onboarding.py`, `src/onboarding_full.py`) e moduli in `src/pipeline/`, `src/semantic/`, `src/tools/`.
...
```
**Nota**: gli **orchestratori sono in `src/`**, non in `src/pipeline/`. `pipeline/` contiene **solo** moduli riusabili.
...
- **Niente `print()`** in produzione. Usare `logging` con configurazione centralizzata (**unico file log**, es. `onboarding.log`).
...
- [ ] Lint/format ok (black/ruff/isort)
- [ ] Test locali ok (`pytest -q`)
- [ ] Nessun `print()` residuo; logging coerente
- [ ] Nessun segreto in diff; `.env.sample` aggiornato
- [ ] README/SUMMARY e docs allineati (se toccati)
- [ ] Comandi interattivi/non‑interattivi testati
