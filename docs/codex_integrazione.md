...
- **ui/AGENTS.md:** vincoli per `onboarding_ui.py` e componenti di interfaccia; build e test controllati, riscritture solo atomiche.
- **orchestrators/AGENTS.md:** tutela dei flussi di orchestrazione, regole sui prompt, nessuna modifica invasiva ai workflow.
- **services/semantic.api/AGENTS.md:** principio “DB first”: SQLite come SSoT per i tag; YAML ammesso solo in modalità legacy con nota di migrazione.
...
- **Scenario Chat:** brainstorming, piani di refactor, revisione architetturale (non scrive/né esegue).
