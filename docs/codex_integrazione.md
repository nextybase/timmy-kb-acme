...
**6) Performance**  Cache RAW PDF auto-invalidata da `safe_write_*` (config in `raw_cache` di `config/config.yaml`); per lNLP usa `--nlp-workers/--nlp-batch-size` e (se serve debug) `--nlp-no-parallel`.
