...
## Decisione
- **Cache RAW eagere**: `safe_write_*` e le copy verso `raw/` invalidano e pre-riscaldano la cache LRU di `iter_safe_pdfs`; TTL configurabile via `TIMMY_SAFE_PDF_CACHE_TTL`.
...
- Se la cache RAW cresce troppo, rivedere `_SAFE_PDF_CACHE_CAPACITY` o rendere dinamico il TTL.
