...
## Decisione
- **Cache RAW eagere**: `safe_write_*` e le copy verso `raw/` invalidano e pre-riscaldano la cache LRU di `iter_safe_pdfs`; TTL/cap configurabili in `config/config.yaml` (`raw_cache`).
...
- Se la cache RAW cresce troppo, rivedere `raw_cache.max_entries`/`ttl_seconds`.
