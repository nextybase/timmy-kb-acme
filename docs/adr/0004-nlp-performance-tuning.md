...
## Decisione
- **Cache RAW eagere**: `safe_write_*` e le copy verso `raw/` invalidano e pre-riscaldano la cache LRU di `iter_safe_pdfs`; TTL/cap configurabili in `config/config.yaml` (`raw_cache`), override via `TIMMY_SAFE_PDF_CACHE_TTL` e `TIMMY_SAFE_PDF_CACHE_CAPACITY`.
...
- Se la cache RAW cresce troppo, rivedere `raw_cache.max_entries`/`ttl_seconds` o i rispettivi override ENV.
