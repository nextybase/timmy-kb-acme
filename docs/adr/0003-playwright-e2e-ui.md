...
## Decisione
Adottiamo **Playwright (Chromium headless)** per esercitare i percorsi UI critici in modalita end-to-end. I test girano in CI e locale con un runner dedicato (`pytest -m "e2e"`), usando Streamlit in modalita headless e gli stub applicativi (`PREVIEW_MODE=stub`, percorsi isolati tramite fixture `tmp_path`). Le dipendenze `playwright` e `pytest-playwright` sono aggiunte a `requirements-dev.in` per garantire installazioni riproducibili.

## Alternative considerate
- **Selenium / webdriver classici**: maggiore complessita di setup, necessita di driver specifici per OS, lentezza nei run deterministici.
...
- Riesaminare se in futuro servira coprire browser multipli (Firefox/WebKit) o se l'introduzione di componenti React richiedera tool aggiuntivi.
