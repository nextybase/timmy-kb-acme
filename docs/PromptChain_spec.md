...
### 3.1 Phase 0 – Analytical, read-only prompts (Prompt 0, 0a..0x)
...
### 3.2 Phase 1..N – Operational micro-PR prompts
...
### 3.3 Prompt N+1 – Final QA and chain closure
...
## 5. Prompt Template Expectations
...
## 6. Protocol Violations (do not do)
...
## 7. Part A – Governance (Planner + OCP)

### 7.1 Actors
- **Planner:** defines the business goals, constraints, and success criteria; it does not modify the repository or run commands.
- **OCP (OrchestratoreChainPrompt):** translates Planner directives into numbered prompts, enforces this spec (sections 2‑5), ensures each prompt covers one turn, and never edits files.
...
### 7.2 Onboarding Task Codex (mandatory entry point)
...
### 7.3 Prompt Chain definition
- The chain is a numbered, turn-based sequence (Prompt 0…N+1) with a clear scope for each prompt as per section 5.
...
- **Execution:** OCP issues prompts sequentially; Codex responds with one micro-PR (diff + report + intermediate QA), then waits for the next prompt. Each operational response includes the memo from section 5 and confirms compliance with the turn-based protocol.
...
## 8. Part B – Operational Contract for Codex

### 8.1 What is a Prompt Chain for Codex?
...
- Each prompt corresponds to a micro-PR defined in the template from section 5 and obeys the mandatory language policy from section 4.
...
- The Active Rules memo from section 5 must appear at the start of every operational response.

### 8.4 Required output per prompt
...
## 9. Codex Smoke Chain – Diagnostic Test
