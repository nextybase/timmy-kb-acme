...
- YAML_STRUCTURE_FILE – **override opzionale** del file YAML con la struttura delle cartelle (default: `config/cartelle_raw.yaml`; fallback: `src/config/cartelle_raw.yaml`)
-----
...
1. *«Eseguire il push su GitHub?»* (default **No**). Se accetti, la pipeline verifica la presenza di GITHUB_TOKEN e propone il branch di destinazione di default (letto da GIT_DEFAULT_BRANCH, es. main), consentendoti di confermarlo o modificarlo.
1. **Pulizia finale** –
...
- **Alias deprecati:** **non più supportati dalla v1.0.4**. Usare i flag ufficiali `--no-drive` e `--no-push`.
